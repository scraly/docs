...
```bash
helm install ovh-cert-lab jetstack/cert-manager --namespace cert-manager --create-namespace --version v1.6.1 -f files/01-cert-manager/01-cert-manager-definition.yaml
```
...
__/!\ Warning /!\\__  
Use the Let's Encrypt Staging environment is not recommended and not compliant with this tutorial.  

```bash
kubectl --namespace cert-manager apply -f files/01-cert-manager/02-acme-production-issuer-http01.yaml
```
...
__For information:__  
A `PersitentVolume` will be created to host all PostgreSQL data.  
...
```bash
helm install ovh-keycloak-lab codecentric/keycloak -n keycloak --create-namespace --version 15.1.0 -f files/02-keycloak/01-keycloak-definition.yaml
```
